...
static f32 SmoothStepF(f32 a, f32 b, f32 x)
{
    f32 t = Math::Clamp((x - a) / (b - a), 0.0f, 1.0f);
...
f32 TerrainField::Height(f32 x, f32 z) const
{
    ...
    f32 mmask = SmoothStepF(0.50f, 0.62f, base);                   // 山脉集中在内陆
...
Vec3 TerrainField::PlanarNormal(f32 x, f32 z, f32 eps) const
{
...
// 归一化: 海平面以下 → <0.5, 最高山 → ~1
f32 TerrainField::PlanarNormalized(f32 x, f32 z) const
{
    f32 h = Height(x, z);
    return Math::Clamp(0.5f + h / 780.0f, 0.0f, 1.0f);
}

// ---------------------------------------------------------------------------
// 球形转换: 立方体六面映射 (与 PlanetGrid FACES 一致)
//   face 0: +X  u→+Y  v→+Z      face 1: -X  u→+Z  v→+Y
//   face 2: +Y  u→+Z  v→+X      face 3: -Y  u→+X  v→+Z
//   face 4: +Z  u→+X  v→+Y      face 5: -Z  u→+Y  v→+X
// ---------------------------------------------------------------------------
static const i8 FACE_U[6] = {1, 2, 2, 0, 0, 1};
static const i8 FACE_V[6] = {2, 1, 0, 2, 1, 0};
static const i8 FACE_S[6] = {1, -1, 1, -1, 1, -1};

Vec3 TerrainField::CubeFacePoint(i32 face, f32 u, f32 v)
{
    f32 a = u * 2.0f - 1.0f;
    f32 b = v * 2.0f - 1.0f;
    Vec3 p{0.0f, 0.0f, 0.0f};
    i32 axis = face >> 1;
    if (axis == 0) p.x = (f32)FACE_S[face];
    else if (axis == 1) p.y = (f32)FACE_S[face];
    else p.z = (f32)FACE_S[face];
    if (FACE_U[face] == 0) p.x = a;
    else if (FACE_U[face] == 1) p.y = a;
    else p.z = a;
    if (FACE_V[face] == 0) p.x = b;
    else if (FACE_V[face] == 1) p.y = b;
    else p.z = b;
    f32 len = sqrtf(p.x * p.x + p.y * p.y + p.z * p.z);
    return {p.x / len, p.y / len, p.z / len};
}

void TerrainField::CubeFaceBasis(i32 face, Vec3& outU, Vec3& outV)
{
    outU = {0.0f, 0.0f, 0.0f};
    outV = {0.0f, 0.0f, 0.0f};
    if (FACE_U[face] == 0) outU.x = 1.0f;
    else if (FACE_U[face] == 1) outU.y = 1.0f;
    else outU.z = 1.0f;
    if (FACE_V[face] == 0) outV.x = 1.0f;
    else if (FACE_V[face] == 1) outV.y = 1.0f;
    else outV.z = 1.0f;
}

void TerrainField::PointToFace(const Vec3& dir, i32& outFace, f32& outU, f32& outV)
{
    ...
    outFace = face;
    outU = comps[FACE_U[face]] * 0.5f + 0.5f;
    outV = comps[FACE_V[face]] * 0.5f + 0.5f;
}

f32 TerrainField::SphereHeight(const Vec3& unitDir) const
{
    f32 so = (f32)mSeed * 0.137f;
    f32 x = unitDir.x + so;
    f32 y = unitDir.y + so * 1.3f;
    f32 z = unitDir.z + so * 0.7f;

    f32 base = Fbm3(x * 2.3f, y * 2.3f, z * 2.3f, 5);              // 0..1 大陆
    f32 mmask = SmoothStepF(0.50f, 0.62f, base);
    f32 ridge = Ridged3(x * 5.2f, y * 5.2f, z * 5.2f, 4);
    f32 hills = Fbm3(x * 9.5f, y * 9.5f, z * 9.5f, 3);

    f32 h = (base - 0.5f) * 2.0f * SPHERE_AMP * 0.62f;             // 大陆 ±4
    h += mmask * powf(ridge, 1.7f) * SPHERE_AMP;                   // 山脉 ~6.5
    h += (hills - 0.5f) * 2.0f * SPHERE_AMP * 0.22f;               // 细节 ±1.4
    return h;
}

Vec3 TerrainField::SphereNormal(const Vec3& unitDir, f32 radius, f32 eps) const
{
    // 沿切平面两个方向做中心差分
    Vec3 ref = fabsf(unitDir.y) > 0.92f ? Vec3{1.0f, 0.0f, 0.0f} : Vec3{0.0f, 1.0f, 0.0f};
    Vec3 t1 = {
        ref.y * unitDir.z - ref.z * unitDir.y,
        ref.z * unitDir.x - ref.x * unitDir.z,
        ref.x * unitDir.y - ref.y * unitDir.x
    };
    f32 l1 = sqrtf(t1.x * t1.x + t1.y * t1.y + t1.z * t1.z);
    t1 = {t1.x / l1, t1.y / l1, t1.z / l1};
    Vec3 t2 = {
        t1.y * unitDir.z - t1.z * unitDir.y,
        t1.z * unitDir.x - t1.x * unitDir.z,
        t1.x * unitDir.y - t1.y * unitDir.x
    };

    auto surf = [&](const Vec3& d) {
        f32 h = SphereHeight(d);
        return Vec3{d.x * (radius + h), d.y * (radius + h), d.z * (radius + h)};
    };
    Vec3 px = surf({unitDir.x + t1.x * eps, unitDir.y + t1.y * eps, unitDir.z + t1.z * eps});
    Vec3 mx = surf({unitDir.x - t1.x * eps, unitDir.y - t1.y * eps, unitDir.z - t1.z * eps});
    Vec3 pz = surf({unitDir.x + t2.x * eps, unitDir.y + t2.y * eps, unitDir.z + t2.z * eps});
    Vec3 mz = surf({unitDir.x - t2.x * eps, unitDir.y - t2.y * eps, unitDir.z - t2.z * eps});
    ...
    // 保证朝外
    if (n.x * unitDir.x + n.y * unitDir.y + n.z * unitDir.z < 0.0f)
    {
        n = {-n.x, -n.y, -n.z};
    }
    f32 len = sqrtf(n.x * n.x + n.y * n.y + n.z * n.z);
    if (len < 1e-8f) return unitDir;
    return {n.x / len, n.y / len, n.z / len};
}

f32 TerrainField::SphereNormalized(const Vec3& unitDir) const
{
    return Math::Clamp(0.5f + SphereHeight(unitDir) / (SPHERE_AMP * 1.35f), 0.0f, 1.0f);
}

f32 TerrainField::Detail3(const Vec3& unitDir) const
{
    f32 so = (f32)mSeed * 0.137f;
    return Fbm3((unitDir.x + so) * 13.0f, (unitDir.y + so * 1.3f) * 13.0f,
                (unitDir.z + so * 0.7f) * 13.0f, 3);
}
...
// ---------------------------------------------------------------------------
Color TerrainField::TerrainColor(f32 hNorm, f32 slope, f32 detail)
{
    ...
    const RGB deepWater{0.055f, 0.13f, 0.29f};
    ...
    {
        c = mix(deepWater, shallow, (hNorm - 0.40f) / 0.08f);
    }
    else if (hNorm < 0.505f)
    {
        c = mix(shallow, sand, (hNorm - 0.48f) / 0.025f);
    }
    else if (hNorm < 0.53f)
    {
        c = sand;
    ...
    {
        f32 t = (hNorm - 0.53f) / 0.15f;
        RGB g = detail > 0.55f ? mix(grass, forest, SmoothStepF(0.55f, 0.75f, detail)) : grass;
        c = mix(sand, g, SmoothStepF(0.0f, 0.25f, t));
    }
    else if (hNorm < 0.80f)
    {
        f32 t = (hNorm - 0.68f) / 0.12f;
        c = mix(grass, rock, SmoothStepF(0.15f, 0.75f, t));
    }
    else
    {
        f32 t = (hNorm - 0.80f) / 0.20f;
        c = mix(rock, snow, SmoothStepF(0.25f, 0.85f, t));
    }

    // 陡坡露岩
    f32 rockW = SmoothStepF(0.45f, 0.8f, slope);
    if (hNorm > 0.505f)
    {
        c = mix(c, rock, rockW * 0.85f);
    }
    // 细节明度扰动
